...
### 翻译原则
1. **技术术语**: 专业术语和代码保持英文（Repository、API、hook、middleware、XSS、CSRF 等），无需添加中文注释或括号解释
2. **普通语句**: 直接翻译为中文，不保留原英文
3. **品牌名称**: 保留英文（Claude Code、GitHub、npm 等）
4. **代码示例**: 代码本身保持不变，代码中的注释翻译为中文
5. **禁止格式**: 不使用 `english(中文)` 或 "中文 // English" 的混合形式
...
- 不要改变代码示例的结构
- 不要删除英文技术术语
- 不要使用 `english(中文)` 或 "中文 // English" 的混合形式
- 翻译完成后不要执行 git commit
